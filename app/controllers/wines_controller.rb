...
before_action :only_strains_availables, only: %i[ new edit ]

  # GET /wines or /wines.json
  def index
    @wines = Wine.eager_load(:strain)      #nos permite tener las cepas precargadas y esten listas para mostrarlas
    @wines = Wine.all
  end
  ...
  def create
    ...
    respond_to do |format|
      if @wine.save
        @wine.addStrainPercentage(params[:wine][:strains_percentage]) 
        #cuando creamos un vino 
  ...
  private
    ...
    def wine_params
      params.require(:wine).permit(:name, :score, { strain_ids: [] })
    end
